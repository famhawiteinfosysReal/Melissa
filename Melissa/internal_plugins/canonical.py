...
try:
    ...
    from famhawiteinfosysReal import WebServer

    _run_canonical = True
except ImportError:
    from Melissa.util.types import WebServer

    _run_canonical = False


from Melissa import command, filters, listener, plugin
from Melissa.core.metrics import MessageStat


class Canonical(plugin.Plugin):
    """Helper Plugin
    This plugin is only available for @dMelissaBot
    to comunicate with https://famhawiteinfosysReal.github.io
    """
    ...
    async def on_load(self) -> None:
        ...
        self._api = WebServer(
            title="Melissa API Docs", description="API Documentation for Melissa Services"
        )
    ...
    async def cmd_login(self, ctx: command.Context):
        """Login to https://famhawiteinfosysReal.github.io"""
        if not self.bot.config.LOGIN_URL:
            return

        await ctx.respond(
            "Click this button to login to Melissa Dashboard",
                    ...
                    [
                        InlineKeyboardButton(
                            ...
                            login_url=LoginUrl(
                                ...
                                forward_text="Login to https://famhawiteinfosysReal.github.io",
