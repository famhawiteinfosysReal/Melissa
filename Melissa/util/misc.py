...
def check_filters(filters: Union[Filter, CustomFilter], Melissa: "Melissa") -> None:
    """Recursively check filters to set :obj:`~Melissa` into :obj:`~CustomFilter` if needed"""
    if isinstance(filters, (AndFilter, OrFilter, InvertFilter)):
        check_filters(filters.base, Melissa)
    if isinstance(filters, (AndFilter, OrFilter)):
        check_filters(filters.other, Melissa)

    # Only accepts CustomFilter instance
    if getattr(filters, "include_bot", False) and isinstance(filters, CustomFilter):
        filters.Melissa = Melissa
