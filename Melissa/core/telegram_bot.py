...
from Melissa.language import get_lang_file
from Melissa.util.cache_limiter import CacheLimiter
...
class TelegramBot(MixinBase):
    ...
    async def init_client(self: "Melissa") -> None:
        ...
        self.client = Client(
            ...
            storage=SQLiteStorage("Melissa"),
        )

    async def start(self: "Melissa") -> None:
        if self.__running:
        ...
        self.log.info("Melissa is ready")
    ...
    async def idle(self: "Melissa") -> None:
        if self.__running:
            raise RuntimeError("This bot instance is already running")
    ...
    async def run(self: "Melissa") -> None:
        if self.__running:
            raise RuntimeError("This bot instance is already running")
    ...
    def update_plugin_events(self: "Melissa") -> None:
    ...
    @property
    def events_activated(self: "Melissa") -> int:
        return len(self._plugin_event_handlers)

    def redact_message(self: "Melissa", text: str) -> str:
    ...
    @cached(ttl=60)
    async def get_chat(self: "Melissa", chat_id: int) -> Union[Chat, ChatPreview]:
        """Wrapper for `Client.get_chat` with a TTL cache."""
        return await self.client.get_chat(chat_id)

    @cached(ttl=60)
    async def get_chat_member(self: "Melissa", chat_id: int, user_id: int) -> ChatMember:
